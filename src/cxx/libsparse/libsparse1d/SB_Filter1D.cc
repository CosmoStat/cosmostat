...
{
    // optimized code :
    // - factorization to avoid multiplications in loops
    // - use of pointers instead of table indexation (when possible)
    // - use a pointer on the test_index function to avoid indirections
    register float* pOutput = Output;
    register float* lastF = &F[SizeFilter-1];
    register float* pF = lastF;
    register int j, p, i = N, start = Start_Filter * Step;
    while (i--)
    {        
        j = SizeFilter;
        p = start;
	*pOutput = 0;
        while (j--)
	{
           *pOutput += Input[(*test_index_function)(p, N)] * *pF--;
           p += Step;
	}
	start++;
	pOutput++;
