...
cout << endl << " FF-FFista::run " << endl << endl;
...
// Initial point z=b
	...
	if(No_coarse) // Substract the coarse scale
	{
		...
		Domain->transform(b, xtmp, allocD); 
	...
	else if(Fast) cerr<<"FFISTA";
	else cerr<<"ISTA";
	cerr<<" with mu="<<Mu<<", and "<<MaxNiter<<" iterations.\n##########"<<endl;
	
// Fista algorithm. 
Fast = Decreasing= True;
if(Fast) cerr << "FAST" << endl;
...
//cerr<<"z i nx ny nl nc "<<i<<":"<<z.nx()<<","<<z.ny()<<","<<z.nl()<<","<<z.nc()<<endl;
	// Threshold update
		if(Decreasing) 
		{
			lvl = Threshold * (MaxNiter-i-1.)/(MaxNiter-1.);
			// lvl = 1-i*1.0/float(MaxNiter-1);			// 1..0
			// lvl = (pow((double)lvl,5)+lvl/100.)/1.01;
			// lvl = Threshold*(1+ lvl*(1000-1));
		}
        // else if (Domain->UseMad == true) lvl = 0;
		
	// Save current solution

        if (Adj == True) 
        {
           if(i!=0)Domain->adjoint_recons(xtmp,z);
        }
        else
        {
...
//cerr<<"zz i nx ny nl nc "<<i<<":"<<z.nx()<<","<<z.ny()<<","<<z.nl()<<","<<z.nc()<<endl;
	...
	// Proximal operator : soft thresholding
        ...
        if (Domain->UseMad == true)
        {
            ...
            Domain->soft_threshold(x,Threshold,No_coarse);
...
//cerr<<"z i nx ny nl nc "<<i<<":"<<z.nx()<<","<<z.ny()<<","<<z.nl()<<","<<z.nc()<<endl;
	...
	if(No_coarse) Domain->add_coarse_scale(x,CS);
	Domain->recons(x,z);
	// sprintf(filename,"%s.fits",NameOut);
    // writefltarr(filename, z);
    fits_write_fltarr(NameOut, z);
...
// Initial point
    ...
    //sprintf(filename,"%sx_%05d.fits",NameOut,iter);
	...
	cerr<<"##########\nBegin ";
	if(Decreasing) cerr<<"MCA";
	else if(Fast) cerr<<"FFISTA-CF";
	else cerr<<"ISTA";
	cerr<<" with mu="<<Mu<<", and "<<MaxNiter<<" iterations.\n##########"<<endl;
    Decreasing = True;
    if (Decreasing) cout << "Decreasing" << endl;
...
//cerr<<"zreal nx ny nl nc "<<zreal.nx()<<","<<zreal.ny()<<","<<zreal.nl()<<","<<zreal.nc()<<endl;
        if (Adj == True) 
        {
           if (iter!=0) Domain->adjoint_recons(xtmp,zreal);
        }
        else
        {
...
//cerr<<"zreal nx ny nl nc "<<zreal.nx()<<","<<zreal.ny()<<","<<zreal.nl()<<","<<zreal.nc()<<endl;
		...
		// sprintf(filename,"%s_%05d.fits",NameOut,iter);
        // writefltarr(filename, zreal);
	...
	if(Positivity) for(int k=0;k<zreal.n_elem();k++) zreal(k) = zreal(k) *(zreal(k)>0);
	// sprintf(filename,"%s_recons.fits",NameOut);
    // writefltarr(filename, zreal);
    fits_write_fltarr(NameOut, zreal);

	for(int i=0;i<z.nx();i++) 
    for(int j=0;j<z.ny();j++) 
    for(int k=0;k<z.nz();k++) z(i,j,k) = complex_f(zreal(i,j,k),0.);
	cerr<<"##########\nEnd.\n##########"<<endl;
}
