...
#ifdef _OPENMP  
   #pragma omp parallel for private(i,j,X,Y) shared(Data1,Data2)
#endif
   for (i = 0; i < Nl; i++) 
   for (j = 0; j < Nc; j++) 
   {
...
void FFTN_2D::fftn2d(cfarray &Im_out, Bool Reverse, bool normalize)
{
    int Nl = Im_out.ny();
    int Nc = Im_out.nx();
    ...
    float *FFT_Dat = (float *)  &(Im_out(0,0));
    Icomplex_f IDat;
    IDat.alloc(Im_out.buffer(), Im_out.ny(),Im_out.nx());
    if (CenterZeroFreq == True) uncenter(IDat);
    transform2d(FFT_Dat, Nc, Nl, Reverse, normalize);
    if (CenterZeroFreq == True) center(IDat);
}  

/******************************************************************************/


void FFTN_2D::fftn2d(cdarray &Im_out, Bool Reverse, bool normalize)
{
    int Nl = Im_out.ny();
    int Nc = Im_out.nx();
