...
void im_sigma_block(Ifloat &Data, Ifloat &Ima_Sigma, int SizeBlock, int Nit, int ResolMad)
{
  ...
  int Kb,Ke,Lb,Le;
  int Nlb = Nl / ResolMad;
  if (Nlb * ResolMad != Nl) Nlb ++;
  int Ncb = Nc / ResolMad;
  if (Ncb * ResolMad != Nc) Ncb ++;
  ...
  for (j=0; j< Ncb; j++)
  {
      for (It = 0; It < Nit; It++)
      {
         ...
         for (l=Lb; l<= Le; l++)
         {
            // if ((k >= Data.nl()) || (l >= Data.nc()) || (k<0) || (l < 0) )
            // {
            //    cout << "ERROR: " << k << " " << l << endl;
            //     exit(-1);
            // }
  ...
  for (j=0; j< Nc; j++)
  {
      // if ((i/ResolMad >= Nlb) || (j/ResolMad >= Ncb)  )
      // {
      //     cout << "ERROR: " << i/ResolMad  << " " << Nlb  << endl;
      //     cout << "ERROR: " << j/ResolMad  << " " << Ncb  << endl;
      //    exit(-1);
      // }
      Ima_Sigma(i,j) = Sig(i/ResolMad,j/ResolMad);
  }
