...
*/
  ...
  int nx() const { return pto_TabNaxis[0];}
  int ny() const { return pto_TabNaxis[1];}
  int nz() const { return pto_TabNaxis[2];}
  ...
  int nc() const { return pto_TabNaxis[0];}
  int nl() const { return pto_TabNaxis[1];}
...
inline PARAM_TYPE& to_array<PARAM_TYPE,ARRAY_TYPE>::operator() (int x, int y) const {
   ...
   if (i_NbAxis != 2)
   {
      cout << "Error: naxis = " << i_NbAxis << " and a two dim array is expected ..." << endl;
      exit(-1);
   }
#endif
...
#endif
  register int indx = (*test_index_function)(x,pto_TabNaxis[1]);
  register int indy = (*test_index_function)(y,pto_TabNaxis[0]);
...
#endif
   register int indx = (*test_index_function)(x,pto_TabNaxis[0]);
   register int indy = (*test_index_function)(y,pto_TabNaxis[1]);
...
#else
  PARAM_TYPE* buf = &po_Buffer[0];
  PARAM_TYPE* otherBuf = &pro_Mat.po_Buffer[0];
  int N = i_NbElem;
  while (N--) *buf++ = *otherBuf++;
#endif
...
void to_array<PARAM_TYPE,ARRAY_TYPE>::set_attrib () {
  ...
  JunkVar=0;
  Border = I_CONT;
  test_index_function = test_index_cont;
}
