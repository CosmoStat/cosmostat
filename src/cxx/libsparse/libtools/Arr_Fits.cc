                      ...
                      Bool NoBscale)
{
  ...
  /* open the file */
	...
	char * NameFits= fitsname(filename);//FCS added to prevent memory leak
    if ( fits_open_file(&fptr, NameFits, (int) READWRITE, &status) )
...
void fits_read_block(char *filename, fltarray & Image, int Indi, int Indj, Bool NoBscale)
{
  ...
  /* open the file */
    status = 0;         /* initialize status before calling fitsio routines */
	char * NameFits= fitsname(filename);//FCS added to prevent memory leak
    if ( fits_open_file(&fptr, NameFits, (int) READONLY, &status) )
         FitsPrintError( status );
	free(NameFits);
    hdunum = 1;  /*read  table */
    if ( fits_movabs_hdu(fptr, hdunum, &hdutype, &status) ) /* move to the HDU */
...
void fits_read_dblarr(char *File_Name, dblarray & Data)
{
    ...
    char FitsName[MAXCHAR];
 	char * NameFits= fitsname(File_Name);//FCS added to prevent memory leak
...
void fits_write_cfarr2d(char * filename, cfarray &Mat)
{
    // Header
	long naxis=3;
	long naxes[3]; naxes[0]=Mat.nx(); naxes[1]=Mat.ny();  naxes[2]=2;
 	int status;
	int simple;
	int bitpix=-32;
	simple   = True;
    ...
    fltarray Data;
    Data.alloc(Mat.nx(),  Mat.ny(), 2);
    for(int j=0;j<Mat.ny();j++)
    for(int i=0;i<Mat.nx();i++)
    {
        Data(i,j,0) =    Mat(i,j).real();
        Data(i,j,1) =    Mat(i,j).imag();
    }
    fits_write_fltarr(filename, Data);
}

/************************************/

void fits_read_cfarr2d(char * filename, cfarray &Mat)
{
    fltarray Data;
    fits_read_fltarr(filename,  Data);
    Mat.alloc(Data.nx(),  Data.ny());
    for(int j=0;j<Mat.ny();j++)
    for(int i=0;i<Mat.nx();i++)
    {
        // Mat(i,j).real() = Data(i,j,0);
        // Mat(i,j).imag() = Data(i,j,1);
        Mat(i, j) = complex_f(Data(i,j,0), Data(i,j,1));
    }
