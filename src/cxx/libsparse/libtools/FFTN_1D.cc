...
#ifdef USE_FFTW
void FFTN_1D::fftw_fftn1d(fltarray &Signal, complex_f *Buff, Bool Reverse, bool normalize)
{
    float* inSignal=Signal.buffer();
    size_t Nelem=Signal.n_elem();
    fftwf_plan p;
    //Manque Normalization
    //Manque centering
    fftwf_complex* out = reinterpret_cast<fftwf_complex*>(Buff);
    if(Reverse) p = fftwf_plan_dft_c2r_1d(Nelem,out, inSignal,FFTW_ESTIMATE);
    else p=fftwf_plan_dft_r2c_1d(Nelem,inSignal,out,FFTW_ESTIMATE);
    fftwf_execute(p);
