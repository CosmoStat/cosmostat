...
void MatOper::inv_mat_svd(dblarray &Mat, dblarray &InvMat, double minT)
// MATRIX inversion: number of lines >= number of columns
// InvMat = Mat^-1
// Mat: IN = matrix with P = Mat.ny  and Q = Mat.nx   (P >= Q)
//  This is an updated version of the previous function, modified to
// handle the thresholding of small eigenvalues.
// If wmax is the largest eigenvalue, all eigenvalue below minT * wmax will be set to 0
{
	void dsvdcmp(double **a, int m, int n, double w[], double **v);
	int i,j;
	dblarray  M1,tMat;
	...
	for(i=0; i < Q; i++) Diag(i,i) = EV(i);
	double wmax= EV.max();
	double EVT= wmax*minT;
	for(i=0; i < Q; i++) if (EV(i) < EVT)
	{
		Diag(i,i) = 0.;
	}
