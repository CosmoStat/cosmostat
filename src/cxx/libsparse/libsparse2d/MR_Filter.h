...
#define DEFAULT_MAX_ITER_INPAINTING 10
  ...
  void init_param(type_noise TypeNoise, type_filter FilterMethod)
   {
      ...
      Max_Inpainting_Iter = DEFAULT_MAX_ITER_INPAINTING;
      Filter = FilterMethod;
   ...
   int Max_Inpainting_Iter; // Maximum number of iteration when there are missing data (inpainting)
   ...
   void filter(Ifloat &Imag, Ifloat &Result);
