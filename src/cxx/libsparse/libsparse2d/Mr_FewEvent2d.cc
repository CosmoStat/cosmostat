...
void event2d_one_scale (Ifloat &   EventImage, 
                        int         s, 
                        Ifloat &       EventCount, 
                        type_border Border,
                        Bool        WriteAllInfo) {
    ...
    for (int i=0; i<Nl; i++) 
    {
        ...
        int j,k;
        for (k =-Size; k <= Size; k++) 
        for (j =-Size; j <= Size; j++)   Total += EventImage(i+k, j, Border);
        EventCount(i,0) = Total;
        for (j =1; j < Nc; j++) 
        {
            for (k = -Size; k <= Size; k++) 
            {
                Total -= EventImage(i+k,j-Size-1,Border);
                Total += EventImage(i+k,j+Size, Border);
            }
...
void event2d_set_support(MultiResol&       Mr2d_Data, 
    ...
    // number of event
    Ifloat EventCount (Nl,Nc,"event2d_set_support");
    ...
    for (int j=0;j<Nc;j++) 
    {
            ...
            //
            int NEventReal = (int) (EventCount(i,j) + 0.5);
...
void mr2d_psupport(MultiResol&     Mr2d_Data, 
   ...
   for (int s=0; s< NScale-1; s++) {
      Mr2d_NoiseModel.CFewEvent2d->find_threshold (
			...
			WriteAllInfo);
      event2d_set_support(Mr2d_Data, s, Mr2d_NoiseModel.Event_Image,  Border, Mr2d_NoiseModel.CFewEvent2d->mThreshold,  Mr2d_NoiseModel, WriteAllInfo);
   }
}
