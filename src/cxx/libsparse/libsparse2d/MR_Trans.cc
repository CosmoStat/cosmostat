...
static void mr_transform (Ifloat &Image, MultiResol &MR_Transf, Bool EdgeLineTransform, type_border Border, Bool Details)
{
    ...
    switch (MR_Transf.Type_Transform)
    {
       ...
       case TO_PAVE_BSPLINE:
            ...
            for (s = 0; s < Nbr_Plan -1; s++)
            {
               MR_Transf.band(s).setBorder( Border );
...
//            break;
       ...
       case TO_LC:
        ...
        case TC_FCT:
	    {
	       ...
	       {
                 int nbrBand = FCT->nbr_band(s);

                 for (int b=0; b < nbrBand; b++)
	         {
                    int nl = FCT->size_band_nl(s,b);
                    int nc = FCT->size_band_nc(s,b);
	            for (int i=0; i < nl; i++)
		    for (int j=0; j < nc; j++)
		    {
...
// 	       D -= Image;
// 	       INFO_X(D, "ERROR REC = ");
	       // cout << "ERROR REC = " << D.INFO_X("ERR") << endl;
 	       //cout << "ETRANS CP " << endl;
	    }
	    break;
       case TO_PYR_MEYER:
