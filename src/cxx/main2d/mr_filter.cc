...
char Name_Write_Sup[512];          /* output support file name */
char MaskFileName[512];          /* output support file name */
...
int Max_Inpainting_Iter = DEFAULT_MAX_ITER_INPAINTING; /* Maximum number of iteration in case of inpainting */
...
Bool Verbose=False;
Bool GetEgde=False;
Bool MaxInpaintingIterOpt=False;
...
{
    cout << endl << endl << argv[0] << ": " << Soft.banner() << endl;
    ...
    fprintf(OUTMAN, "         [-I MaskFileName]\n");
    fprintf(OUTMAN, "             Filename of the mask containing the bad data (Mask[i,j]=1 for good pixels and 0 otherwise.\n");
    ...
    fprintf(OUTMAN, "         [-Q SNRFileName]\n");
    fprintf(OUTMAN, "             SNR Filename; gives the SNR of each coefficient. Valid only with -m10 option.\n");
    vm_usage();
    verbose_usage();
...
static void filtinit(int argc, char *argv[])
{
    ...
    /* get options */
    while ((c = GetOpt(argc,argv, (char *) "I:HQ:AD:M:U:C:B:u:pbm:f:t:T:g:c:n:s:i:e:w:kE:S:N:F:KPR:vzZ:G:odh")) != -1)
    {
	switch (c)
        {
	  case 'H': MissingData = True;  break; // MadCenter = True;
	       break;
      case 'I': if (sscanf(OptArg,"%s", MaskFileName) != 1)
            {
	  ...
	  case 'B':
	   ...
	   case 's':
        ...
        if (sscanf(OptArg,"%d,%d", &Max_Iter, &Max_Inpainting_Iter) == 2) MaxInpaintingIterOpt=True;
        else if (sscanf(OptArg,"%d",&Max_Iter) != 1)
        {
...
if (Verbose == True)
{
    ...
    if (MaxInpaintingIterOpt == True) cout << "Max_Inpainting_Iter = " << Max_Inpainting_Iter << endl;
    ...
    if (UseNSigma  == True)
    {
        double Nsig12 = erffc( (double) 12. / sqrt((double) 2.) ) / 2.;
        ...
        for (s=0; s <  NbrBand; s++) ModelData.TabEps[s] = (ModelData.NSigma[s] < 12) ? erffc( (double)  ModelData.NSigma[s] / sqrt((double) 2.) ) / 2.: Nsig12;
    }
    else for (s=0; s <  NbrBand; s++) ModelData.TabEps[s] = EpsilonPoisson;
...
//    else cout << "KO" << endl;
    ...
    if (MissingData == True)
    {
        ...
        {
            io_read_ima_float(MaskFileName, CFilter.MaskIma);
            if ((CFilter.MaskIma.nl() != DataB.nl()) || (CFilter.MaskIma.nc() != DataB.nc() ))
            {
                cout << "Error: Mask size = " << CFilter.MaskIma.nl() << ", " <<CFilter.MaskIma.nc() << endl;
                cout << "       Data size = " << DataB.nl() << ", " << DataB.nc() << endl;
                exit(-1);
            }
        }
        BadPixel = True;
    ...
    if (MaxInpaintingIterOpt == True) CFilter.Max_Inpainting_Iter = Max_Inpainting_Iter;
    ...
    if ((Stat_Noise == NOISE_EVENT_POISSON) && (ProbMR == True))
     {
        int b,i,j;
...
// 	cout << "xerf(1e-30) = " <<  sqrt(2.) * ABS(dierfc((1e-30))) << endl;
        ...
        b = LastScale;
        for (i=0; i< MR_Data.size_band_nl(b); i++)
        for (j=0; j< MR_Data.size_band_nc(b); j++) MR_Data(b,i,j)=0;
	   ...
	   //          << "," << j << ")=" << Coef << std::endl;
           ...
           double Coef = ModelData.prob_signal_few_event( MR_Data(b,i,j),  b, i, j ) ;
           //std::cout << setprecision( 16 ) << "prob(" << b << "," << i
	   //          << "," << j << ")=" << prob << std::endl;

           if (b < LastScale)
           {
               if (Coef < 1e-35)  Coef = (float) (sqrt((double) 2.) * fabs(dierfc((1e-35))));
