...
long int StochasticLib3::WalleniusNCHyp (long int n, long int m, long int N, double odds) {
/*
...
long int StochasticLib3::WalleniusNCHypUrn (long int n, long int m, long int N, double odds) {
...
long int StochasticLib3::WalleniusNCHypTable (long int n, long int m, long int N, double odds) {
  // Sampling from Wallenius noncentral hypergeometric distribution 
  ...
  // the same parameters.
  static long int wnc_n_last = -1, wnc_m_last = -1, wnc_N_last = -1; // previous parameters
...
long int StochasticLib3::WalleniusNCHypRatioOfUnifoms (long int n, long int m, long int N, double odds) {
  // sampling from Wallenius noncentral hypergeometric distribution 
  // using ratio-of-uniforms rejection method.
  static long int wnc_n_last = -1, wnc_m_last = -1, wnc_N_last = -1; // previous parameters
  static double wnc_o_last = -1;
  static long int wnc_bound1, wnc_bound2;// lower and upper bound
  ...
  if (n != wnc_n_last || m != wnc_m_last || N != wnc_N_last || odds != wnc_o_last) {
    ...
    if (!UseChopDown) {
      ...
      // find safety bounds
      wnc_bound1 = (long int)(mean - 4. * wnc_h);
      if (wnc_bound1 < xmin) wnc_bound1 = xmin;
      wnc_bound2 = (long int)(mean + 4. * wnc_h);
...
long int StochasticLib3::WalleniusNCHypInversion (long int n, long int m, long int N, double odds) {
  // sampling from Wallenius noncentral hypergeometric distribution 
  ...
  x1 = (long int)(wnch1.mean());       // start at floor and ceiling of mean
  x2 = x1 + 1;
  xmin = m+n-N; if (xmin<0) xmin = 0;  // calculate limits
  xmax = n;     if (xmax>m) xmax = m;
...
void StochasticLib3::MultiWalleniusNCHyp (long int * destination, 
...
******************************************************************************/
long int StochasticLib3::FishersNCHyp (long int n, long int m, long int N, double odds) {
/*
...
long int StochasticLib3::FishersNCHypInversion 
...
*/ 
  static long int fnc_n_last = -1, fnc_m_last = -1, fnc_N_last = -1;
...
long int StochasticLib3::FishersNCHypRatioOfUnifoms 
(long int n, long int m, long int N, double odds) {
/* 
  Subfunction for FishersNCHyp distribution. 
...
*/ 
  static long int fnc_n_last = -1, fnc_m_last = -1, fnc_N_last = -1; // previous parameters
  static double fnc_o_last = -1;
  static long int fnc_bound;           // upper bound
  ...
  if (n != fnc_n_last || m != fnc_m_last || N != fnc_N_last || odds != fnc_o_last) {
    ...
    // find safety bound
    fnc_bound = (long int)(mean + 4.0 * fnc_h);
...
***********************************************************************/
void StochasticLib3::MultiFishersNCHyp (long int * destination, 
long int * source, double * weights, long int n, int colors) {
/*
