...
class Ylmgen
  ...
  private:
    ...
    void recalc_recfac (int m)
      ...
      double f_old=1;
      for (int l=m; l<(int) recfac.size(); ++l)
        {
  ...
  public:
    ...
    Ylmgen (int l_max, int m_max, double epsilon=1e-30)
      ...
      for (int m=0; m<(int) cf.size(); ++m)
        cf[m] = ldexp(1.,(m+minscale)*large_exponent2);

      mfac[0] = 1;
      for (int m=1; m<(int) mfac.size(); ++m)
        mfac[m] = mfac[m-1]*sqrt((2*m+1.)/(2*m));
      for (int m=0; m<(int) mfac.size(); ++m)
        mfac[m] = inv_ln2*log(inv_sqrt4pi*mfac[m]);
      for (int l=0; l<(int) t1fac.size(); ++l)
        t1fac[l] = sqrt(4.*(l+1)*(l+1)-1.);
      for (int i=0; i<(int) t2fac.size(); ++i)
        t2fac[i] = 1./sqrt(i+1.);
      }
