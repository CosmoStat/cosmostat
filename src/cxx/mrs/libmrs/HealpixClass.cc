...
int  CAlmR::hard_threshold(float T, int & MaxNonZeroL)
{
    ...
    // cout << "hard_threshold:Thres =  " << T << ", Keep " <<  Cpt <<  endl;
    return Cpt;
}
...
void CAlmR::read(char *Name, int Nside, bool Fast)
{
    ...
    // cout << "PB ALMREAD FILE " << fitsname(Name) << " " << Fast << endl;

    get_almsize (infile,lmaxIn,dummy);
    // cout << "ALMREAD FILE infile " << infile <<  " lmaxIn=" << lmaxIn << ",nside = " <<  Nside << endl;

    alloc (Nside, lmaxIn, Fast);
    // cout << " Lmax() = " << Lmax() << endl;

    read_Alm_from_fits(infile, *this, Lmax(),Mmax(),2);
    // cout << "ALMREAD FILE " << fitsname(Name) <<  " " << Lmax() << "  " << lmaxIn << endl;
     FastALM = Fast;
     AllocNside = Nside;
}
...
void CAlmR::info(char *Name)
{
	    ...
	    cout << Name << " Re ==> Min = " << A.min() << ", Max = " << A.max() << ", Sig = " << A.sigma() << endl;
        cout << Name << " Im ==> Min = " << B.min() << ", Max = " << B.max() << ", Sig = " << B.sigma() << endl;
}

// ===========================

void CAlmR::alm_trans(Hmap<REAL> & map)
{
	//map.info();
	...
	if (Norm == true)
	{
	    if (Verbose == true ) cout << "alm_trans Norm = " << NormVal << " " <<  Lmax() << endl;
	    for (int l=0; l <= Lmax(); l++)
...
void CAlmR::alm_rec(Hmap<REAL> & Map, bool KeepAlm, int RecNside)
{
    // cout << "IN Almrec,  nside = " << AllocNside << " " << AllocNside <<  " " << " Max = " <<  max_absalm() << endl;
    dblarray A;
    if (KeepAlm == true)
    {
    	A.alloc(Lmax()+1,Lmax()+1,2);
    ...
    double offset = (*this)(0,0).real()/sqrt(fourpi);
    // xcomplex<REAL> Z =  (*this)(0,0);
    
	(*this)(0,0) = 0;
	if  (RecNside != 0) Map.SetNside(RecNside, RING);
	else Map.SetNside(AllocNside, RING);

	// if (Verbose == true) cout << "Almrec nside = " << Map.nside() << " " << AllocNside << endl;     
    alm2map(*this,Map);
    Map.Add(offset);
    ...
    if (KeepAlm == true)
    {
	    ...
	    for (int m=0; m <= l; m++) 
	    {
	    	 (*this)(l,m) = xcomplex<REAL> (A(l,m,0), A(l,m,1));
	    }
    }

    // (*this)(0,0) = Z;
   // cout << "OUT Almrec,  "<< " Max = " <<  max_absalm() << endl;
}
...
void CAlmR::wiener(float SigmaNoise)
{
   ...
   WienerFilter(0) = 1.;
    WienerFilter(1) = 1.;
   if (Verbose == true)  cout << "wiener: " << Lmax() << " " << Mmax() << endl;
    for (int l=2; l <= Lmax(); l++)
	{
