...
static void usage(char *argv[])
{
    ...
    fprintf(OUTMAN, "         [-M]\n");
    fprintf(OUTMAN, "             Median of Absolute Deviation (MAD) denoising.  \n");

    fprintf(OUTMAN, "         [-K]\n");
    fprintf(OUTMAN, "             Kill last scale.  \n");
    
    fprintf(OUTMAN, "         [-C]\n");
    fprintf(OUTMAN, "             CMB removal.  \n");
...
static void sinit(int argc, char *argv[])
{
    int c; 
    /* get options */
    while ((c = GetOpt(argc,argv,(char *) "KMn:s:Cg:l:vzZ")) != -1) 
    {
	switch (c)  { 
         case 'K': KillLastScale = true; break;
         case 'C': CMBDel = true; break; 
...
int main(int argc, char *argv[])
{
   ...
   int Lmax = mrs_get_lmax (Lmax,  Nside, 0.0);
    ...
    if (CMBDel == true)
    {
        ...
        KillLastScale=true;
        NSigma=5;
        NbrScale = log((double) Nside) / log((double) 2.);
    }
