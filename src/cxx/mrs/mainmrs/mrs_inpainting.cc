   ...
   { 
	      ...
	      Err = 0.;
   	      for (int p=0; p < Result.Npix(); p++) 
   	      {
              ...
              Err = sqrt(Err / (float) Result.Npix());
	      
            int CptAlm=0;
		      
		   // Resi.info((char*) "     --> BefW MAP");
           
           ALM.Set(ALM_FIRST_L,ALM_FIRST_L);
	   ALM.alm_trans(Resi);
           MaxAlm =ALM.max_absalm() / SigmaNoise;
	   
   	      // cout << FirstSoftThreshold << " " << LastSoftThreshold  << endl;
       	  Lambda =  LastSoftThreshold  + DeltaThreshold  *(1.-erf(2.8 * (i-Fi)/ Ni));  // 4 versus 2.8  in IDL code
	  if (MaxAlm < Lambda)  
	  {
	     Fi = i;
	     Lambda = MaxAlm * 0.95;
	     Ni = Max_Iter - i;
	     FirstSoftThreshold = Lambda;
	     DeltaThreshold = FirstSoftThreshold - LastSoftThreshold;
	     Lambda =  LastSoftThreshold  + DeltaThreshold  *(1.-erf(2.8 * (i-Fi)/ Ni));
	  }
      ...
      //  ALM.info((char *) "ALM After");
	   ...
	   // ALM_FIRST_L = MaxNonZeroL;
           // ALM.Set(ALM_FIRST_L,ALM_FIRST_L);
           ALM.alm_rec(Resi);
	    
	    
           // ALM.Set(Lmax_Tot,Lmax_Tot);
       ...
       // Resi.info((char*) "     -> AftW MAP");
            ...
            if (MaxNonZeroL > 1)
	    {
 	    ...
 	    ALM_FIRST_L = MIN(2*MaxNonZeroL, Lmax_Tot);
           ...
           // sprintf(NN, "xx_res_it_%d.fits", i+1);
           // Result.write(NN);
  	   if (Verbose == True) Result.info("  RES");
     }
