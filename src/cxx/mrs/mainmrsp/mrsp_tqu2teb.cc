...
{
    fprintf(OUTMAN, "Usage: %s options PolaMapTQU_File_input PolaMapTEB_file_output \n\n", argv[0]);
...
{
	//Call "mrsp_tqu2teb file_in file_out"
	
	sinit(argc, argv);
	
	PolaHdmap Map_TQU;
	
	Map_TQU.read( Name_file_Map1 );// Read a map in TQU
	
	Map_TQU.swap_tqu_teb( PolaFastALM );
	
	Map_TQU.write( Name_file_Map2 );// Write map in TEB
