...
**    			In this file, "degrade_mask2d" stands for A in the case of
**    			missing data, using the array "Mask".
**    		
**    calling example : 
**    		program_name --mu 1 --tolvar 1e-6 -g 0.01 -i 200 -n 7 noisy_image mask out
**    		program_name --mu 1 --tolvar 1e-6 -g 0.01 -i 200 -n 7 noisy_imagze_with_0_values out
**    		
******************************************************************************/
...
void degrade_mask2d(fltarray &x, bool degrade, bool reverse)
{
	if(degrade) x *= Mask;
}
...
//cerr<<"data size nx ny "<<Data.nx()<<","<<Data.ny()<<endl;

// Input mask
	if(estim_mask)
	{
		// We set Mask = (|data|<1e-6)
		Mask.alloc(Data.nx(),Data.ny(),0);
		int nx = Data.nx(),ny = Data.ny();
		for(int j=0;j<ny;j++) for(int i=0;i<nx;i++)
			Mask(i,j) = abs(Data(i,j,0)) > 1e-6; //( (Data(i,j,0) != 0) && (Data(i,j,1) != 0) ); 
	}
	else
	{
...
// Filters for the wavelet transforms and TV
	...
	// Fista algorithm
		degrade_mask2d(Data, !use_init, false);// degrade only if not using init
		F.run(Data, Recons, &degrade_mask2d);
		degrade_mask2d(Recons, false, true);
	}
	...
	// Nesterov algorithm
		degrade_mask2d(Data, !use_init, false);// degrade only if not using init
		N.run(Data, Recons, &degrade_mask2d);
	}
	else 
	{
