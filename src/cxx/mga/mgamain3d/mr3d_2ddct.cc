...
*******************************************************************************
**
**    DESCRIPTION  2D dct on each frame (z-plane)
**    ----------- 
**                 
******************************************************************************/
...
static void filtinit(int argc, char *argv[])
{
	...
	/* get options */
	while ((c = GetOpt(argc,argv,(char*)"In:d:vFRScg:a:s:f:i:Cx:O")) != -1) 
	{
		switch (c) 
		{
			case 'I': 
...
// Get long options
	...
	it = opts.find("--mask");
	if(it!=opts.end()){ istringstream ss(it->second); ss>>Name_Mask; }
	it = opts.find("--BS");
	if(it!=opts.end()){ istringstream ss(it->second); ss>>BlockSize; }
	it = opts.find("--skiporder");
...
// Print parameters
	...
	if(Name_Mask[0]!='\0')
	{
		...
		InvMask.alloc(Mask.nx(),Mask.ny(),Mask.nz()); InvMask.init(1.);
		InvMask -= Mask;
	}
	...
	if(Compute_forward)
	{
		...
		DCTF.alloc(Data.nx(), Data.ny(), Data.nz(), BlockSize, BlockOverlap);
		DCTF.set_skip_order(skip_order);
		DCTF.transform(Data, TabBand, true);
	}
	else // We load the transformed cube
	{
		sprintf(filename,"%s.mr",Name_Imag_In);
		fits_read_fltarr(filename, TabBand);
		
		if(Name_Mask[0]!='\0')
			Orig = TabBand*Mask;
		
		DCTF.alloc(TabBand.nx(),TabBand.ny(),TabBand.nz(), BlockSize, BlockOverlap);
		DCTF.set_skip_order(skip_order);
...
// Thresholding
	if( SigmaNoise>=0 )
		DCTF.threshold(TabBand, SigmaNoise, NSigma, FilterType);
...
// Reconstruction
	if (Compute_recons)
	{
		if(Name_Mask[0]!='\0')
		{
			...
			{
				TabBand = Orig+InvMask*TabBand;
				DCTF.recons(TabBand, Data);
				DCTF.transform(Data, TabBand, false);
			}
		}
