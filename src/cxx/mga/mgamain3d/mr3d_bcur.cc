...
type_pmeyer3D Type_PMW3D = DEF_TYPE_PMW3D;
...
//					 12345678901234567890123456789012345678901234567890123456789012345678901234567890
    ...
    fprintf(OUTMAN, "             1 : Coherent with gaussian transform,\n");
...
static void filtinit(int argc, char *argv[])
{
	...
	/* get options */
	while ((c = GetOpt(argc,argv,(char*)"n:vFRScO:B:b:w:g:a:s:f:i:")) != -1) 
	{
		switch (c) 
		{
			case 'n': 
			...
			case 'b': 
				...
				CurTrans=(type_linelet3d_WTtrans)temp;
				break;
			case 'w': 
				if (sscanf(OptArg,"%d",&temp) != 1) 
...
// Get long options
	...
	it = opts.find("--poisson");
	if(it!=opts.end()){ istringstream ss(it->second); ss>>temp; Type_PMW3D = (type_pmeyer3D)temp; }
...
// Logical links between inputs
	...
	// BCurvelet initialisation	
		...
		DataC->set_no_recons(!Compute_recons);
		DataC->set_3sigma(Use3sig);
		DataC->set_no_coarse(no_coarse);
		DataC->set_keep_energy(keep_energy);
		DataC->init(Data.nx(), Data.ny(), Data.nz(), NbrScale3D, BlockSize, TypeW3D, SB1D);

	// Transform, filtering and reconstruction
		DataC->filter(Data, Recons, SigmaNoise, NSigma, FilterType, Alpha, WienerBS, force4sigma, UseCubeSigma, Name_Imag_Out);
	...
	// Get Band
	//	DataC->get_band(1,4,TabBand, Band);
	...
	// Thresholding
		if( SigmaNoise>=0 )
		{
			...
			else if(FilterType==FT_WIENER) DataC->wiener(TabBand, SigmaNoise*NSigma/3., 3, UseCubeSigma);
	...
	// Reconstruction
		if(Compute_recons) DataC->recons(TabBand, Recons);
