...
static void filtinit(int argc, char *argv[])
{
	...
	/* get options */
	while ((c = GetOpt(argc,argv,(char*)"In:N:d:vFRScg:a:s:f:i:Cx:")) != -1) 
	{
		switch (c) 
		{
			case 'I': 
...
// Print parameters
	...
	if(Compute_forward)
	{
		...
		CurTF.set_kill_time_fine(0);
		CurTF.alloc_from_coarse(NbrScale, NbrScale1D, Data.nx(), Data.ny(), Data.nz(), NbrDir2d, False, False, (Bool)!Complex_transform);
		if(SigmaNoise>=0) CurTF.get_norm_coeff(3);
		CurTF.transform(Data, TabBand, true);
	}
...
// Statistics
	if( Noise_calib )
		CurTF.noise_calib(TabBand, Name_Imag_Out);
	if( Extract_stat )
		CurTF.extract_stat(TabBand, Name_Imag_Out);
	
// Thresholding
	if( SigmaNoise>=0 )
		CurTF.threshold(TabBand, SigmaNoise, NSigma, FilterType);
	
	/*
	(TabBand[0][20][0])(10,15,100)=1;
	(TabBand[0][20][3])(7,2,80)=1;
	(TabBand[0][20][5])(3,10,20)=1;
	(TabBand[1][3][0])(5,1,40)=1;
	(TabBand[1][3][3])(2,4,30)=1;
	(TabBand[1][3][5])(1,7,10)=1;
	(TabBand[2][10][0])(0,4,10)=1;
	(TabBand[2][10][3])(1,3,4)=1;
	(TabBand[2][10][5])(3,2,15)=1;
	*/
