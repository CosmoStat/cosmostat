...
// Print parameters
	...
	fltarray Data;
	char filename[64];
	FCur_SFrame *CurSF = new FCur_SFrame();
	Ifloat **TabBand;
	...
	if(Compute_forward)
	{
		...
		CurSF->alloc_from_coarse(NbrScale, Data.nx(), Data.ny(), Data.nz(), NbrDir2d, False, False, (Bool)!Complex_transform);
		if(SigmaNoise>=0) CurSF->get_norm_coeff(3);
		CurSF->transform(Data, TabBand, true);
	}
...
// Statistics
	if( Noise_calib )
		CurSF->noise_calib(TabBand, Name_Imag_Out);
	if( Extract_stat )
		CurSF->extract_stat(TabBand, Name_Imag_Out);
	
// Thresholding
	if( SigmaNoise>=0 )
		CurSF->threshold(TabBand, SigmaNoise, NSigma, FilterType);
