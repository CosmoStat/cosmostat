...
*/
	...
	fprintf(OUTMAN, "         [--no-adjoint] Prevents the use of adjoint\n");
	fprintf(OUTMAN, "             reconstruction is then done by simple sum of scales.\n");
	manline();
...
// Get long options
	...
	it = opts.find("--no-adjoint");
	if(it!=opts.end()){ istringstream ss(it->second); use_adjoint=false; }
...
// Get short options
	...
	if (Verbose == True)
	{
		...
		if(!use_adjoint) cout << "Do not use the adjoint" << endl;
...
// forward transform
	...
	// UOWT initialisation
		atrou->set_use_adjoint(use_adjoint);
...
// Thresholding
	if( SigmaNoise>0 )
	{
		if(FilterType==FT_HARD || FilterType==FT_SOFT)
			atrou->threshold(TabBand, SigmaNoise*NSigma, FilterType==FT_SOFT);
