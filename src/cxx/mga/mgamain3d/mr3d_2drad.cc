...
// Print parameters
	...
	if(Compute_forward)
	{
		...
		RadF.alloc(Data.nx(), Data.ny(), Data.nz());
//		if(SigmaNoise>=0) RadF.get_norm_coeff(NSigma);
		RadF.transform(Data, TabBand, true);
	}
	else // We load the transformed cube
	{
		...
		RadF.alloc(TabBand.nx(),TabBand.nx(),TabBand.nz());
...
// Thresholding
//	if( SigmaNoise>=0 )
//		RadF.threshold(SigmaNoise, NSigma, FilterType);
	...
	if (Compute_recons)
	{
		if(Name_Mask[0]!='\0')
		{
			...
			for(int i=0;i<niter;i++)
			{
				...
				RadF.recons(TabBand, Data);
				RadF.transform(Data, TabBand, false);
			}
		}
