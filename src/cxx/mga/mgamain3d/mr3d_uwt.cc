...
// Get short options
	...
	SB1D->Border = I_PERIOD; // Period for a perfect reconstruction, mirror/cont for applications
...
// forward transform
	...
	// UOWT initialisation
		uwt->init(TabBand, Data.nx(), Data.ny(), Data.nz(),NbrScale3D);

	// Forward Transform
		uwt->transform(Data,TabBand,NbrScale3D);
	}
	else // We load the transformed cube
	{
	// read data
		Name_Imag_In = add_mr(Name_Imag_In);
		uwt->read(Name_Imag_In,TabBand);
	}
	
// Statistics
	if(Extract_stat) 
		uwt->extract_stat(TabBand);

// Thresholding
	if( SigmaNoise>0 )
	{
		if(no_coarse) uwt->set_no_coarse(true);
		if(no_fine) uwt->set_no_fine(true);
		if(FilterType==FT_HARD || FilterType==FT_SOFT)
			uwt->threshold(TabBand, SigmaNoise, NSigma, FilterType, force4sigma);
		else if(FilterType==FT_WIENER) uwt->wiener(TabBand, SigmaNoise, 3);
...
// Output transform
	...
	// Save the transform
		...
		uwt->write(filename, TabBand);
		/*
		for(int i=0;i<7*(NbrScale3D-1)+1;i++)
		{
			sprintf(filename,"%s_band_%d.fits",Name_Imag_Out,i);
			writefltarr(filename, TabBand[i]);
		}
		*/
	}
...
// Compute and output reconstruction
	if(Compute_recons)
	{
		if(positivity) uwt->set_positivity(true);
		uwt->recons(TabBand,Recons,NbrScale3D);
		uwt->apply_positivity(Recons);
