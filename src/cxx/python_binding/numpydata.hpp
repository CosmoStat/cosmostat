...
py::array_t<complex<float>> tab2d_2array_2d(const Ifloat &image){

  auto array = py::array_t<complex<float>>(image.n_elem());
  auto buffer = array.request();
  complex<float> *pointer = (complex<float> *) buffer.ptr;

  for (int i=0; i<image.nl(); i++) {
    for (int j=0; j<image.nc(); j++) {
      pointer[j + i * image.nc()] = (complex<float>) image(i, j);
    }
  }
