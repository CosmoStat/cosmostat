...
int main (int argc,char *argv[])
{
    ...
    { 
       ...
       if (CenterZeroFreq == True)
           cout << "Zero frequency at the middle of the image" << endl;
       else cout << "Zero frequency at the bottom left" << endl;
    } 
    ...
    for(int i=0;i<Data.ny();i++) cData(i,j) = complex_f( Data(i,j), 0);
 
       // Fourier transform 
    FFTN_2D CF;
    Bool Reverse = (Dir == 1) ? False: True;
    CF.CenterZeroFreq = CenterZeroFreq;
    CF.fftn2d(cData, Reverse);   // bool normalize=false
