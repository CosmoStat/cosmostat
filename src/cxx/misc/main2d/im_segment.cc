...
static void infinit(int argc, char *argv[])
{
    ...
    /* get options */
    while ((c = GetOpt(argc,argv,"bvzZ:")) != -1) 
    {
	switch (c) 
        {
...
int main(int argc, char *argv[])
{
    ...
    Segment.alloc(Dat.nl(),Dat.nc(), "Segment");
    int NLabel=0;

    im_segment (Dat, Segment, NLabel, Threshold, KillBorder);
    io_write_ima_float (Name_Imag_Out, Segment);
    if (Verbose == True)
    {
