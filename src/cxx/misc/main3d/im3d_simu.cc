...
static void siminit(int argc, char *argv[])
{
	...
	if (OptInd < argc)
      ...
      {
          fprintf(OUTMAN, "\n\nError: noise addition option (pgc) or (and) convolution option has to be selected \n");
          exit(-1);
      }
...
int main(int argc, char *argv[])
{
    ...
    Nz = Dat.nz();
    int FrameSize = Nl*Nc;
    DatOut.alloc(Nc,Nl,Nz);
    for (int f=0; f < Nz; f++)
    {
        unsigned RandVal = InitRnd*f;
        DataB.alloc( Dat.buffer()+FrameSize*f, Nl, Nc);
        Result.alloc(DatOut.buffer()+FrameSize*f, Nl, Nc);
		...
		im_noise_gaussian(Result, Noise_Ima, RandVal);
     		Result += DataB;
      		break;
    	   case NOISE_POISSON:
      		/* add a poisson noise */
 		Result = DataB;
		im_noise_poisson (Result, 1., RandVal);
      		break;
    	   case NOISE_GAUSS_POISSON:
     	 	/* add a poisson noise and then a gaussian noise */
 		Result = DataB;
		im_noise_poisson (Result, Gain, RandVal);
		im_noise_gaussian(DataB, Noise_Ima, RandVal);
     		Result += DataB;
       		break;
    	   default:
    ...
    Header.origin = Cmd;
    io_3d_write_data (Name_Imag_Out, DatOut, &Header);
    exit(0);
}
