...
int main(int argc, char *argv[]) {
  ...
  // compute
  o_Result.res_FirstCompute (); o_Result.res_Recons ();
...
void pca_Param::operator () (int argc, char *argv[]) {
  ...
  while ((c = GetOpt(argc,argv,"t:n:WkvpwLCx:O:F:K:u:T:")) != -1) {
...
void pca_Param::trace () {
   ...
   for (int i=0;i<MAX_NBR_PCA_IMA;i++)
      if (tti_TabKill[i][0] > 0)
         cout << " -- [-K:] : eigen not used : " << tti_TabKill[i][0] << endl; 
...
void pca_Result::res_FirstCompute () {
  ...
  if (po_Param->e_WriteTransf) {
     char NameEigen[256];
     for (int k=0; k < po_Param->i_NbSpectre; k++) {
        sprintf(NameEigen, "pca_Transf_%d",k+1);
...
void pca_Result::res_Recons () {
   ...
   o_Mr1dPca.invsubtransform (po_TabMr1d, po_TabMr1d, po_Param->ti_NbrUseEigen,
