...
char Name_MR1_Out[256]; /* output file name */
...
static void usage(char *argv[])
{
    ...
    fprintf(OUTMAN, "Usage: %s options image output1 output2\n\n", argv[0]);
...
static void filtinit(int argc, char *argv[])
{
	...
	if (OptInd < argc) strcpy(Name_MR1_Out, argv[OptInd++]);
         else usage(argv);
...
int main(int argc, char *argv[])
{
    ...
    MultiResol MR_Entr(Imag.nl(), Imag.nc(),Nbr_Plan,Transform, "MRNoiseModel");
    MR_Data.Border = I_MIRROR;
    MR_Entr.Border = I_MIRROR;
    
    if (Verbose == True) cout << "Noise modeling " << endl;
    ModelData.model(Imag, MR_Data);
    MR_Data.band(NbrBand-1).init();

    for (s=0;s < NbrBand-1; s++)
    for (i=0; i< MR_Data.size_band_nl(s); i++)
    for (j=0; j< MR_Data.size_band_nc(s); j++)  MR_Entr(s,i,j) = MR_Data(s,i,j);
    
     
    // calculates the entropy of each wavelet coefficient
    if (Verbose == True) cout << "Entropy calculation " << endl;
    ModelData.prob(MR_Entr);
    for (s=0;s < NbrBand-1; s++)
    {
       if (Verbose == True)
       {
          cout << "Entropy Band " << s+1 << endl;
       }
       for (i=0; i< MR_Data.size_band_nl(s); i++)
       for (j=0; j< MR_Data.size_band_nc(s); j++) 
       {
          if (MR_Entr(s,i,j) < 1e-7)  MR_Entr(s,i,j) = - log(1e-7);
          else MR_Entr(s,i,j) = - log(MR_Entr(s,i,j));
       }
    }
    MR_Entr.write(Name_MR_Out);
      
      
    // caclulate: -  p(w is signal) * log( p(w) )
    ModelData.prob_noise(MR_Data, True);
    for (s=0;s < NbrBand-1; s++)
    {
       if (Verbose == True)
       {
          cout << "Signal Entropy Band " << s+1 << endl;
       }
       for (i=0; i< MR_Data.size_band_nl(s); i++)
       for (j=0; j< MR_Data.size_band_nc(s); j++) 
       {
          MR_Data(s,i,j) *= MR_Entr(s,i,j);
       }  
    }  
    MR_Data.write(Name_MR1_Out);
    
    // entropy per scale
    fltarray EntrScale(NbrBand);
    fltarray EntrSigScale(NbrBand);
    fltarray Npt(NbrBand);
    EntrScale.init();
    EntrSigScale.init();
    Npt.init();
    ...
    for (j=0; j< MR_Data.size_band_nc(s); j++) 
    {
       EntrScale(s) += MR_Entr(s,i,j);
       EntrSigScale(s) += MR_Data(s,i,j);
       Npt(s) += 1.;
    }    
    ...
    for (s=0;s < NbrBand-1; s++)
    {
       ...
       {
          EntrScale(s) /= Npt(s);
          EntrSigScale(s)  /= Npt(s);
       }
       
       cout << "Band " << s+1 << "  Entropy = " << EntrScale(s) << endl;
       cout << "        Entropy Signal = " << EntrSigScale(s) << endl << endl;
    }
    exit(0);
} 
