...
float Noise_Ima=7.;              /* noise standard deviation */
...
static type_comp TabMethod[NBR_OK_METHOD] = {COMP_MRMEDIAN,COMP_MALLAT};

Bool ReadInPseudo=False;
...
//     manline();  
    ...
    fprintf(OUTMAN, "         [-p ReadPseudo]\n");
    fprintf(OUTMAN, "              Read the input data as a pseudo image (JPEG only).\n");
    manline();  
...
//     fprintf(OUTMAN, "         [-l]\n");
//     fprintf(OUTMAN, "              Compress the residual. Default is no.\n");
//     manline();      
...
//     
//     fprintf(OUTMAN, "         [-N]\n");
//     fprintf(OUTMAN, "             Use noise modeling. Default is no.\n");
...
//    while ((c = GetOpt(argc,argv,"pm:vs:n:rc:g:R:NB:zZ:")) != -1) 
     while ((c = GetOpt(argc,argv,"B:vn:g:zZ:")) != -1) 
   {
	switch (c) 
        {
           case 'p': ReadInPseudo = (ReadInPseudo==True) ? False: True;break;
           case 'B': 
		...
		UseBudget = True;
		break;
           case 'N':NoiseInData = True;Noise_Ima=0;break;
 	   case 'm':
...
int main(int argc, char *argv[])
{
    ...
    CompIma.ReadInPseudo = ReadInPseudo;
