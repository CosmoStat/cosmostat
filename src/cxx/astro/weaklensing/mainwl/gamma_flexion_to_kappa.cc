...
char Name_flexion[200];
char Name_shear[200];
char Name_kappaE[200];
char Name_kappaB[200];
...
{
    fprintf(OUTMAN, "Usage: %s options pixel_size gamma_in flexion_in kappaE_out \n\n", argv[0]);
    fprintf(OUTMAN, "   where [pixel_size] = arcmin and options =  \n");
...
static void infinit(int argc, char *argv[])
{
    ...
    if (OptInd < argc) strcpy(Name_shear, argv[OptInd++]);
    else usage(argv);
...
int main(int argc, char **argv) {
    ...
    fits_read_fltarr(Name_shear,shear_In);
    ...
    for(int y=0; y < Ny; y++){
        ...
        gamma1(x,y) = shear_In(x,y,0);
        gamma2(x,y) = shear_In(x,y,1);
    }
    
    ShearInversions inv(max(Zero_pad,2*max(Nx,Ny)));
    inv.gamma_flexion2kappa(pixel_size, gamma1,gamma2, F1,F2,kappaDbl);
