...
class MCA1D {
  public:
   // General parameters
   Bool UseMadForLambdaEstimation;
   ...
   //-----------------------------------------------
   PAVE_1D_WT *WT; // Pointer to the wavelet transform class
   ...
   void aldct_proj(fltarray& SigRec, float Lamba_Sigma, int Iter);
   ...
   Bool UseMask;
	 ...
	 float CFAR_NsigmaMad;
   Bool CFAR;          // If true, the threshold at each iteration is derived from
                       // T= MAD(Coef)*CFAR_NsigmaMad
   Bool CFDR;          // If true, the threshold at each iteration is derived from
                       // from the FDR. 
   float CFDR_Qparam;   // Parameter used in the FDR calculation.
		       // if CFAR or CFDR is True then the variable
		       //  UseMadForLambdaEstimation is set to True.
   		       
   
   // Parameter for the atrous algorithm
   // ----------------------------------
   int AT_FirstDetectScale;
   Bool AT_SuppressIsolatedPixel;
   Bool AT_OnlyPositivDetect;
   ...
   int WT_NbrUndecimatedScale;          // Number of scales
   Bool WT_PositivRecSig;      // If true, impose positive constraint
   fltarray WT_Trans;           // Wavelet bases decomposition
   Bool WT_OnlyPositivDetect;
   Bool WT_SuppressIsolatedPixel;
   int WT_FirstDetectScale;
   Bool WT_NonOrthoFilterBank;
   ...
   void infoVerbose (char *nameSignalIn, char *nameSignalOut);
