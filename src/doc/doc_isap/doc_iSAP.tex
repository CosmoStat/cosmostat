...
\def\sparse2d{Sparse2D/}
\def\projsparse2d{Sparse2D/V1.0 }
\def\defprojsparse2d{Sparsity for 1D and 2D data set}

\def\msvst{MSVST/}
\def\projmsvst{MSVST/V1.0 }
\def\defprojmsvst{Multi-Scale Variance Stabilizing Transform for 1D and 2D data set}
...
\def\msvsts{MRS-MSVST/}
\def\projmsvsts{MRS-MSVST/V1.1 }
\def\defprojmsvsts{Multi-Scale Variance Stabilizing Transform on the Sphere}

\def\df{DarthFader/}
\def\projdf{DarthFader/V1.1 }
\def\defprojdf{Spectroscopic Redshift Estimation}

\def\sgal{SparseGal/}
\def\projsgal{SparseGal/V1.0 }
\def\defprojsgal{Sparsity for galaxies survey analysis}

\def\scmb{SparseCMB/}
\def\projscmb{SparseCMB/V1.0 }
\def\defprojscmb{Sparsity for CMB data analysis}
...
\author{CEA-SACLAY/IRFU-AIM \\
 ...
 $ $ \\ {\large CEA Technical Report} \\ 
  $ $ \\
 $ $ {\large http://www.cosmostat.org}
  }
 
 \date{\begin{figure}[htb]
\centerline{
\hbox{
\includegraphics[width=13.5cm]{FIG/fig_ecur.png}
}}
\end{figure}
}
...
\include{msvst_intro}  
\include{msvst_info}  
\include{msvst_denoising}  
\include{msvst_inpainting}  
\include{msvst_background}  
\include{msvst_multichannel}  
\include{msvst_idl}
\include{msvst_ccl}
