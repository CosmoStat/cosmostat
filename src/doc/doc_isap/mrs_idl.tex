...
\item Undecimated wavelet transform on the sphere (using a spline wavelet, Meyer wavelet or Needlet wavelet).
...
\begin{center}
     USAGE: mrs\_trans, InImag, Trans, NbrScale=NbrScale, Alm=Alm, AT=AT, Cur=Cur, UWT=UWT, PyrWT=PyrWT, OWT=OWT, Rid=Rid, DCT=DCT, 
     					lmax=lmax, FirstBlockSize=FirstBlockSize, DifInSH=DifInSH, MeyerWave=MeyerWave, NeedletWave=NeedletWave, B\_NeedletParam=B\_NeedletParam, Overlap=Overlap
\end{center}}
where
\begin{itemize}
...
\item {\em NeedletWave}: int , 1 if the  keyword  NeedletWave  is used, otherwise 0
\item {\em B\_NeedletParam}: int,  B\_NeedletParam (default is 2).
...
\item {\em DEC} : IDL structure, transformation result (depends on the chosen transform).
...
\end{itemize}
If the keyword ALM is set (it should be a structure (see mrs\_almtrans) containg the ALM coefficients of the data, 
...
\subsection{Iterative Wiener filtering of a map: mrs\_itwiener}
\index{IDL routines!mrs\_itwiener}
\index{spherical harmonics! iterative wiener filtering}
Perform wiener filtering of a spherical map when the noise is non-stationary using the Healpix representation 
(nested data representation by default).

If the keyword {\/Frg} is set, it also estimates the contribution coming from extra foreground residuals.
{\bf
\begin{center}
     USAGE: mrs\_itwiener, Imag, Powspec, Recons, Noise=Noise, fNoise=fNoise, niter=niter,cole=cole,StartWiener=StartWiener, NbrScale=NbrScale, BS=BS,Frg=Frg,RMSmoothing=RMSmoothing,tol=tol,verbose=verbose
\end{center}}
where
\begin{itemize}
\item {\em Imag} : Input IDL array of Healpix map, image to be denoised.
\item {\em Powspec} : float or IDL 1D float array: theoretical power spectrum of the image.
\item {\em Recons} : Output IDL array of healpix denoised image.
\item {\em Noise} :Input Noise realization from which the noise statistics are measured.
\item {\em fNoise} : Output Noise realization (apply the same filtering on the data and on the input noise realization).
\item {\em niter} : scalar, number of iterations. Default is 100.
\item {\em Cole} : scalar, if set the Wiener filter is replaced by the Cole filter.
\item {\em StartWiener} : if set, the algorithm is initialized with the global Wiener filter (i.e. in the spherical harmonics).
\item {\em NbrScale} scalar: number of wavelet scales used for analysis. Default is 4.
\item {\em BS} scalar: minimal patch size used to compute the noise variance. Default is 16.
\item {\em Frg} scalar: if set, also estimates the contribution coming from extra foreground residuals. Default is no.
\item {\em RMSmoothing} scalar: if set, applies a smoothing of the estimated noise variance maps. Default is no.
\item {\em tol} : scalar, convergence precision. Default is $1e^{-6}$.
\end{itemize}
...
plot the wavelet transform of the $\textrm{f}^{\textrm{th}}$ face, for $f \in \{0\ldots 11\}$.
\end{itemize}
...
{\bf
\begin{center}
     USAGE: mrs\_wttrans, Imag, Trans, NbrScale=NbrScale, Healpix\_with\_Glesp=Healpix\_with\_Glesp, lmax=lmax, MeyerWave=MeyerWave, NeedletWave=NeedletWave, B\_NeedletParam=B\_NeedletParam, DifInSH=DifInSH
\end{center}}
where 
\begin{itemize}
...
where 
\begin{itemize}
\item {\em Trans}: input IDL structures  (see mrs\_wttrans).
...
by a simple addition of all wavelet scales. Automatically applied if keyword MeyerWave, NeedletWave, or DifInSH were set at the wavelet decomposition.
\end{itemize}
...
and using the "à trous" algorithm. The wavelet transform is applied successively on the  12 faces of the Healpix image. 
...
IF A MASK IS SPECIFIED, THE DATA HAVE TO BE MULTIPLIED BY THE MASK PRIOR TO CALLING THE MRS\_JADE ROUTINE.
...
\item {\em GMCA\_mask} : 1D IDL array of Healpix map. optional mask applied to the data.
