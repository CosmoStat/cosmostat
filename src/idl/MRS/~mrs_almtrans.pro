...
pro mrs_convol, map, Beam_l, Rec, nside_out = nside_out, Alm=Alm
vs = size( Beam_l)
LMax = vs[1]-1
mrs_almtrans, map, Alm, /tab, lmax=lmax
for l=1,Alm.lmax do  alm.alm[l,*,*] = alm.alm[l,*,*] *  Beam_l[l]
  ...
  if keyword_set(HealpixCXX) then begin
	...
	if keyword_set(tab) then begin
	    for i=0,nl-1 do begin
	       A[ Tabl[i], Tabm[i], 0 ] = a.real[i]
	       A[ Tabl[i], Tabm[i], 1 ] = a.imag[i]
	       
	    Alm = dlbarr(lmax, lmax, 2)
	    for l=0,lmax do begin
	    for m=0,l do begin
	       Alm[l,m,0] = a.real[ Tab
	   for i=0, size(l)[1]-1 do begin 
        tab2alm, A,  Alm
       ; Alm = mrdfits(FN2, 1)
       ; vs = size(alm.real)
       ; A = dblarr(vs[1], 2)
