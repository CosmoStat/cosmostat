...
function cl_recons, Map, SigmaNoise, angSpec, filters=bkj, Mask=Mask, Nscale=Nscale, Firstl=Firstl, lmax=lmax, tousil=tousil, WTNscale=WTNscale, Niter=Niter, SDCT=SCDT, SWT=SWT, OptWT=OptWT, BestBasis=BestBasis, Pos=Pos, TabInterval=TabInterval, Cl=Cl, Proj_Resi=Proj_Resi, True_Resi=True_Resi, part_nMSE=part_nMSE, Mcl=Mcl, uresult=uresult
