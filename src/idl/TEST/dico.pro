...
Ima = -1
 IF N_PARAMS() LT 1 THEN BEGIN
   ...
   GOTO, CLOSING
 ENDIF
 ; print, "Block.SizeBlock = ", Block.SizeBlock
...
print, " Number of patches = ", m
print, " Size of patches = ", n
 ...
 for iter=1,niter do BEGIN
    ...
    for i=0,K-1 do D[i,*] = D[i,*]  / sqrt( total(D[i,*]^2))
     ...
     END
      ...
      wset, 1
      load, x
      print, 'Iter = ', Iter, ', Energy = ', E(iter-1)
END ; FOR 
