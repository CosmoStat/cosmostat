...
;------------------------------------------------------------------------------
function mrs_cftoe2latspec, CFToe, wind=wind
...
weight = 1.
if wind eq 'rect' then begin 
  weight = 1.
endif

if wind eq 'hamming' then begin
  w = hanning(Nh, Nh, alpha=0.54)
  weight = Nh^2/total(w^2)
endif

if wind eq 'hanning' then begin 
  w = hanning(Nh, Nh, alpha=0.5)
  weight = Nh^2/total(w^2)
endif

if wind eq 'cosine' then begin 
  w = fltarr(Nh, Nh)
  for i = 0, Nh-1 do begin
    for j = 0, Nh-1 do begin
      ii = (i-Nh/2)/float(Nh)
      jj = (j-Nh/2)/float(Nh)
      w(i,j) = cos(!pi*ii)*cos(!pi*jj)
    endfor
  endfor
  weight = Nh^2/total(w^2)
endif
if wind eq 'bartlett' then begin 
  w = fltarr(Nh, Nh)
  for i = 0, Nh-1 do begin
    for j = 0, Nh-1 do begin
      ii = (i-Nh/2.)/Nh/2.
