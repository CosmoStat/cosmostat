...
Scale = mrs_wtget(Trans, NbrScale-1, NormVal=NormVal)
Scale[*]=0
...
pro planck_mask_mca, TabFN, Mask, DIR=DIR, Unit=Unit
...
for f=0,Nf-1 do begin
  ...
  ; tvs, i-filter, min=-500, max=500, tit='Data - Filter'
   ind = where(filter NE 0, c)
   m512[*] = 0
   if c GT 0 then m512[ind] = 1
   ; m = 1 - m512
  ...
  ; save, filename='xx2.xdr', filter
   mask_mca, i, tabC, SelectTrans=[1,4], niter=40, /expo, LastThreshold=0, SigmaNoise=1, NbrScale=NbrScale,  tabsupport=tabsupport, FirstGuess= FirstGuess, /resi
   ; save, filename='xx_wmap-nilc.xdr', i, filter, tabC
   FN = 'xx_mca_ns5' + strc(f+1) + '.xdr'
  save, filename=FN, i, filter, tabC, m
  FN = 'MCA_' +TabFN[f]  
  mrs_write, FN, tabC[*,1]
end
...
planck_mask_mca, TabFN, Mask, DIR=DIR, Unit=1e6  ; Uni=1e6  to put the data in uK
