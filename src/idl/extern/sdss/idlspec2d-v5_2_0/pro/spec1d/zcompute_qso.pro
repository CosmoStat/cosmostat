...
;------------------------------------------------------------------------------
function zcompute_qso, objflux, objivar, starset, starflux, starmask, nfind=nfind, $
 ...
 plottitle=plottitle, doplot=doplot1, debug=debug, verbose=verbose
   ...
   if (nobj GT 1) then begin
      ...
      for iobj=0L, nobj-1 do begin
         ...
         zans1 = zcompute_qso(objflux[*,iobj], objivar[*,iobj], $
          starset, starflux, starmask, nfind=nfind, $
   ...
   nlag = ((pmax - pmin + 1) / pspace) > 1
     ...
     ; these don't need to be integers for the bspline
   lags = - lindgen(nlag) * pspace + pixoffset - long(pmin) 
