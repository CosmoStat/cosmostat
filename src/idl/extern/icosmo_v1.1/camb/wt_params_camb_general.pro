pro wt_params_camb_general, fid, fileparams=file, verbose=verbose, tk=tk, filetk=filetk, filepk=filepk, filecl=filecl

; March 09 - Modified by AnR. filecl keyword debugged
; Dec 08 - Modified by AnR. Input structure is now fiducial, not cosmo
; Dec 08 - Written by Anais Rassat
;
; PURPOSE: Write a general 'params.ini' file that CAMB (http://camb.info/)
...
; Note this version is slightly different to wt_params_camb.pro which
; is written specifically for a Planck Fisher matrix calculations
; wt_params_camb_general.pro is more general, in that eventually every
; CAMB input value can be changed.
...
if keyword_set(file_cl) then scl_temp = filecl else scl_temp = 'test'
...
omega_cdm = fid.cosmo.omega_m - fid.cosmo.omega_b ; need to also remove neutrinos in the future
ombh2  = strcompress('ombh2          = '+string(fid.cosmo.omega_b*fid.cosmo.h^2))
omch2  = strcompress('omch2          = '+string(omega_cdm*fid.cosmo.h^2))
omk    = strcompress('omk            = '+string(1.d0-(fid.cosmo.omega_m+fid.cosmo.omega_l)))
;for physical=F 
om_cdm = strcompress('omega_cdm      = '+string(omega_cdm))
omb    = strcompress('omega_baryon   = '+string(fid.cosmo.omega_b))
om_de  = strcompress('omega_lambda   = '+string(fid.cosmo.omega_l))
;Others
bigh   = strcompress('hubble         = '+string(fid.cosmo.h*100.))
w0     = strcompress('w0             = '+string(fid.cosmo.w0))
wa     = strcompress('wa             = '+string(fid.cosmo.wa))
n_s     = strcompress('scalar_spectral_index(1)  = '+string(fid.cosmo.n))
transfer_kmax = strcompress('transfer_kmax           = '+string(3./fid.cosmo.h))
if keyword_set(verbose) then begin
   print, om_cdm
   print, omb
   print, om_de
...
printf, 3, ''
printf, 3, 'sigma8 = '+string(fid.cosmo.sigma8)
