...
y2 = self.ysize
Polyfill, [x1, x1, x2, x2, x1], [y1, y2, y2, y1, y1], /Device, Color=FSC_Color(self.color)
...
IF self.count GE self.nsteps THEN BEGIN
   ...
   Polyfill, [0, 0, self.xsize, self.xsize, 0], $
             [0, self.ysize, self.ysize, 0, 0], /Normal, Color=FSC_Color(self.color)
   Widget_Control, self.tlb, Destroy=1
...
autoTimer = Obj_New("ShowProgress", tlb, Color="RED", Steps=20, Delay=5, /AutoUpdate)
...
progressTimer = Obj_New("ShowProgress", tlb, Color="GREEN", /CancelButton)
