...
RETURN, Filepath(Root_Dir=self.directoryName, self.filename)
END ;-------------------------------------------------------------------------------
...
lgeo = Widget_Info(self.filelabelID, /Geometry)
fgeo = Widget_Info(self.filetextID, /Geometry)
bgeo = Widget_Info(self.filebrowseID, /Geometry)
bbgeo = Widget_Info(self.filebaseID, /Geometry)

   ; Add the bits and bobs together.

fmargin = 2*lgeo.margin + 2*fgeo.margin + 2*bgeo.margin + 2*bbgeo.margin
fpadding = lgeo.xpad + fgeo.xpad + bgeo.xpad + bbgeo.xpad
ffixedsize = (lgeo.scr_xsize + bgeo.scr_xsize)

   ; Get the geometries of the directory name widgets.

lgeo = Widget_Info(self.dirlabelID, /Geometry)
fgeo = Widget_Info(self.dirtextID, /Geometry)
bgeo = Widget_Info(self.dirbrowseID, /Geometry)
bbgeo = Widget_Info(self.dirbaseID, /Geometry)

   ; Add the bits and bobs together.

dmargin = 2*lgeo.margin + 2*fgeo.margin + 2*bgeo.margin + 2*bbgeo.margin
dpadding = lgeo.xpad + fgeo.xpad + bgeo.xpad + bbgeo.xpad
dfixedsize = (lgeo.scr_xsize + bgeo.scr_xsize)
...
; I don't know WHY I still need a fudge factor of 11 here,
; but I do on my Windows machine. I'd love to have someone
; explain this to me. :-(

ftextsize = thisSize - (fmargin + fpadding + ffixedsize)
Widget_Control, self.filetextID, Scr_XSize=ftextsize - 11

dtextsize = thisSize - (dmargin + dpadding + dfixedsize)
Widget_Control, self.dirtextID, Scr_XSize=dtextsize - 11
...
PRO FSC_FileSelect::Select_Directory, event
...
dirname = Dialog_Pickfile( Title='Select Directory...', $
...
dirname = StrMid(dirname, 0, length-1)
...
filename = Dialog_Pickfile( File=self.filename, $
                            Filter=self.filter, $
                            Path=self.directoryname, $
                            Get_Path=thePath, $
...
self.dirbaseID = Widget_Base(self.tlb, Row=1, UValue=self)
...
self.dirbrowseID = Widget_Button( self.dirbaseID, $
                               Event_Func='FSC_FileSelect_Event_Handler', $
                               UValue={method:'Select_Directory', object:self}, $
                               Value='Browse')

self.filebaseID = Widget_Base(self.tlb, Row=1)
...
self.filebrowseID = Widget_Button( self.filebaseID, $
                               Event_Func='FSC_FileSelect_Event_Handler', $
...
PRO FSC_FileSelect__Define
   ...
   objectClass = { FSC_FileSelect, $      ; The object class FSC_FILESELECT.
                   ...
                   dirbrowseID: 0L, $     ; The ID of the directory browse button widget.
                   ...
                   filter: "", $          ; The current file filter used in file selection.
...
self->GetProperty, Filename=filename, DirectoryName=directory, Filter=filter, $
   Event_Pro=event_pro, Event_Func=event_func
IF event_pro NE "" OR event_func NE "" THEN BEGIN
    event = {CW_FileSelect, self->GetTLB(), event.top, 0L, self->GetFilename(), $
        filename, filter, directory}
    RETURN, event
ENDIF ELSE RETURN, 0
END ;-----------------------------------------------------------------------------------------------------------------------------
...
IF N_Elements(selectfont) EQ 0 THEN selectfont = ""
...
IF N_Elements(textfont) EQ 0 THEN textfont = ""
