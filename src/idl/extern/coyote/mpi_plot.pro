...
pskeywords = PSConfig(Cancel=cancelled)
...
IF thisEvent EQ 'WIDGET_BASE' THEN BEGIN
   ...
   Widget_Control, (*infoptr).drawID, Draw_XSize=(*infoptr).xminsize > event.x, $
      Draw_YSize=event.y - (*infoptr).ysize
...
PRO MPI_Plot, x, y, _Extra=extra, XTitle=xtitle, YTitle=ytitle, Title=title, $
   Overplot=overplot, Color=color, DataColor=datacolor, PSym=psym
...
ENDIF ELSE BEGIN
   ...
   struct = {MPI_OVERPLOT, Ptr_New(dep), Ptr_New(indep), Ptr_New(extra), color:color, psym:psym}
...
info = {xaxis:xaxis, yaxis:yaxis, wid:wid, dep:dep, indep:indep, plotconfig:plotconfig, $
   ...
   x:!X, y:!Y, p:!p, clearoplotID:clearoplotID}
