...
PRO TVSCALE, image, x, y, KEEP_ASPECT_RATIO=keep, POSITION=position, $
   ...
   TVSCL=tvscl, NORMAL=normal, TOP=top, BOTTOM=bottom, $
   NCOLORS=ncolors, MAXValue=max, MINValue=min, NOINTERPOLATION=nointerp, $
...
ENDIF ELSE BEGIN ; All other devices.
   ...
   CASE true OF
      ...
      0: TV, BYTSCL( CMCONGRID(image, CEIL(xsize), CEIL(ysize), INTERP=interp, $
            MINUS_ONE=minusOne, HALF_HALF=half_half), Top=top-bottom, Max=max, Min=min) + bottom, $
            ROUND(xstart), ROUND(ystart), _EXTRA=extra

      1: IF thisDepth GT 8 THEN BEGIN
            TV, BYTSCL(CMCONGRID(image, 3, CEIL(xsize), CEIL(ysize), INTERP=interp, $
               MINUS_ONE=minusOne, HALF_HALF=half_half), Top=top-bottom, Max=max, Min=min) + bottom, $
               ROUND(xstart), ROUND(ystart), _EXTRA=extra, True=1
         ENDIF ELSE BEGIN
      ...
      2: IF thisDepth GT 8 THEN BEGIN
            TV, BYTSCL(CMCONGRID(image, CEIL(xsize), 3, CEIL(ysize), INTERP=interp, $
               MINUS_ONE=minusOne, HALF_HALF=half_half), Top=top-bottom, Max=max, Min=min) + bottom, $
               ROUND(xstart), ROUND(ystart), _EXTRA=extra, True=2
         ENDIF ELSE BEGIN
      ...
      3: IF thisDepth GT 8 THEN BEGIN
            TV, BYTSCL(CMCONGRID(image, CEIL(xsize), CEIL(ysize), 3, INTERP=interp, $
               MINUS_ONE=minusOne, HALF_HALF=half_half), Top=top-bottom, Max=max, Min=min) + bottom, $
               ROUND(xstart), ROUND(ystart), _EXTRA=extra, True=3
         ENDIF ELSE BEGIN
