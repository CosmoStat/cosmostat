...
;	Wrapping PIOWriteTAB2DObjectIdxMask
...
END
    if (N_ELEMENTS(Index1) EQ 0) THEN     Index1=LONG64(0) ELSE    Index1=LONG64(Index1)
     if (N_ELEMENTS(Index2) EQ 0) THEN     Index2=LONG64(0) ELSE    Index2=LONG64(Index2)
...
MEM_TYP=[-1,1,2,4,4,8,8,-1,-1,16,-1,-1,-1,-1,8,-1]
 PIOWRITETAB2DOBJECTIDXMASK=call_external(PIOLibIDLSO,'piowritetab2dobjectidxmask_tempoidl', $
        data, $
        LONG64(N_ELEMENTS(data)*MEM_TYP(size(data,/type))), $
        Index1, $
        LONG64(N_ELEMENTS(Index1)*MEM_TYP(size(Index1,/type))), $
        Index2, $
        LONG64(N_ELEMENTS(Index2)*MEM_TYP(size(Index2,/type))), $
        Mask, $
 ...
 RETURN,PIOWRITETAB2DOBJECTIDXMASK
END
