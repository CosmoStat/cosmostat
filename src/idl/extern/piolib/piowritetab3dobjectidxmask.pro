...
;	Wrapping PIOWriteTAB3DObjectIdxMask
...
END
    if (N_ELEMENTS(Index) EQ 0) THEN     Index=LONG64(0) ELSE    Index=LONG64(Index)
     ...
     if (N_ELEMENTS(Index3) EQ 0) THEN     Index3=LONG64(0) ELSE    Index3=LONG64(Index3)
...
MEM_TYP=[-1,1,2,4,4,8,8,-1,-1,16,-1,-1,-1,-1,8,-1]
 PIOWRITETAB3DOBJECTIDXMASK=call_external(PIOLibIDLSO,'piowritetab3dobjectidxmask_tempoidl', $
        data, $
        ...
        Index3, $
        LONG64(N_ELEMENTS(Index3)*MEM_TYP(size(Index3,/type))), $
        Mask, $
 ...
 RETURN,PIOWRITETAB3DOBJECTIDXMASK
END
