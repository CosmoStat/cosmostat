FUNCTION PIOREADTAB3DOBJECT,DATA,OBJNAME,TYPE,COMMAND,GROUP, $
...
A = call_external(PIOLibIDLSO,'pioreadxxx_1idl',NBDATA, $
                  ...
                  PIOSTRING2C("TAB3D"),PIOSTRING2C("TAB3D"), $
...
IF (NBDATA GT 0) THEN BEGIN
    ...
    IF (KEYWORD_SET(INDEX) AND NOT KEYWORD_SET(ATIDX)) THEN BEGIN
        IF ('TAB3D' EQ 'TAB2D' OR 'TAB3D' EQ 'IMG2D') THEN BEGIN
            INDEX=LON64ARR(NBDATA)
            INDEX2=LON64ARR(NBDATA)
            MULTI2D=1
        END ELSE BEGIN
            IF ('TAB3D' EQ 'TAB3D' OR 'TAB3D' EQ 'IMG3D') THEN BEGIN
                INDEX=LON64ARR(NBDATA)
                INDEX2=LON64ARR(NBDATA)
