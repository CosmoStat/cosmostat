FUNCTION PIOREADROIOBJECT,DATA,OBJNAME,TYPE,COMMAND,GROUP, $
...
A = call_external(PIOLibIDLSO,'pioreadxxx_1idl',NBDATA, $
                  ...
                  PIOSTRING2C("ROI"),PIOSTRING2C("ROI"), $
...
IF (NBDATA GT 0) THEN BEGIN
    ...
    IF (KEYWORD_SET(INDEX) AND NOT KEYWORD_SET(ATIDX)) THEN BEGIN
        IF ('ROI' EQ 'TAB2D' OR 'ROI' EQ 'IMG2D') THEN BEGIN
            INDEX=LON64ARR(NBDATA)
            INDEX2=LON64ARR(NBDATA)
            MULTI2D=1
        END ELSE BEGIN
            IF ('ROI' EQ 'TAB3D' OR 'ROI' EQ 'IMG3D') THEN BEGIN
                INDEX=LON64ARR(NBDATA)
                INDEX2=LON64ARR(NBDATA)
