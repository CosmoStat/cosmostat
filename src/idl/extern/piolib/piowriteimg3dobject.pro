...
FUNCTION PIOWRITEIMG3DOBJECT,data,Objectname,type,command,MyGroup,INDEX=INDEX,MASK=MASK
    ...
    IF (KEYWORD_SET(INDEX) AND KEYWORD_SET(MASK)) THEN BEGIN
        RETURN,PIOWRITEIMG3DOBJECTIDXMASK(DATA,INDEX,MASK,N_ELEMENTS(INDEX),OBJECTNAME,TYPE,COMMAND,MYGROUP)
    END
    IF (KEYWORD_SET(MASK)) THEN BEGIN 
        RETURN,PIOWRITEIMG3DOBJECTMASK(DATA,MASK,OBJECTNAME,TYPE,COMMAND,MYGROUP)
    END
    IF (KEYWORD_SET(INDEX)) THEN BEGIN 
        RETURN,PIOWRITEIMG3DOBJECTIDX(DATA,INDEX,N_ELEMENTS(INDEX),OBJECTNAME,TYPE,COMMAND,MYGROUP)
    END
 CASE (STRING(TYPE)) OF
...
MEM_TYP=[-1,1,2,4,4,8,8,-1,-1,16,-1,-1,-1,-1,8,-1]
 PIOWRITEIMG3DOBJECT=call_external(PIOLibIDLSO,'piowriteimg3dobject_tempoidl', $
        data, $
