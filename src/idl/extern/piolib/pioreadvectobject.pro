FUNCTION PIOREADVECTOBJECT,DATA,OBJNAME,TYPE,COMMAND,GROUP, $
...
A = call_external(PIOLibIDLSO,'pioreadxxx_1idl',NBDATA, $
                  ...
                  PIOSTRING2C("VECT"),PIOSTRING2C("VECT"), $
...
IF (NBDATA GT 0) THEN BEGIN
    ...
    IF (KEYWORD_SET(INDEX) AND NOT KEYWORD_SET(ATIDX)) THEN BEGIN
        IF ('VECT' EQ 'TAB2D' OR 'VECT' EQ 'IMG2D') THEN BEGIN
            INDEX=LON64ARR(NBDATA)
            INDEX2=LON64ARR(NBDATA)
            MULTI2D=1
        END ELSE BEGIN
            IF ('VECT' EQ 'TAB3D' OR 'VECT' EQ 'IMG3D') THEN BEGIN
                INDEX=LON64ARR(NBDATA)
                INDEX2=LON64ARR(NBDATA)
