FUNCTION PIOREADIMG3DOBJECT,DATA,OBJNAME,TYPE,COMMAND,GROUP, $
...
A = call_external(PIOLibIDLSO,'pioreadxxx_1idl',NBDATA, $
                  ...
                  PIOSTRING2C("IMG3D"),PIOSTRING2C("IMG3D"), $
...
IF (NBDATA GT 0) THEN BEGIN
    ...
    IF (KEYWORD_SET(INDEX) AND NOT KEYWORD_SET(ATIDX)) THEN BEGIN
        IF ('IMG3D' EQ 'TAB2D' OR 'IMG3D' EQ 'IMG2D') THEN BEGIN
            INDEX=LON64ARR(NBDATA)
            INDEX2=LON64ARR(NBDATA)
            MULTI2D=1
        END ELSE BEGIN
            IF ('IMG3D' EQ 'TAB3D' OR 'IMG3D' EQ 'IMG3D') THEN BEGIN
                INDEX=LON64ARR(NBDATA)
                INDEX2=LON64ARR(NBDATA)
