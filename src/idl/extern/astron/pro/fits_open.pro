...
;
; create vectors needed to store header information for each extention
;
        n = 100
        xtension = strarr(n)
        extname = strarr(n)
        ...
        naxis  = lonarr(n)
        axis = lonarr(20,n)
        start_header = lonarr(n)        ; starting byte in file for header
        start_data = lonarr(n)          ; starting byte in file for data
;
; read and process each header in the file if open for read or update
;
        extend_number = 0               ; current extention number being
                                        ; processed
        position = 0L                   ; current byte position in file
