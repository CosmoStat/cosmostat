...
pro IM_DCT, ImagIn, ImagOut, DCT_File_Name=DCT_File_Name, OPT=Opt
ImagIn = float(ImagIn)
if N_PARAMS() LT 2 then begin 
        spawn, 'im_dct'
        print, 'CALLING SEQUENCE: im_dct, ImagIn, ImagOut, DCT_File_Name=DCT_File_Name, OPT=Opt'
        goto, DONE
        end
...
if keyword_set(DCT_File_Name) then filename = DCT_File_Name $
else filename = 'xx_temp.fits'
...
com = 'im_dct ' + Opt + ' ' + NameImag + ' ' +  filename
spawn, com
