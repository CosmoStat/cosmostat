...
pro IM_RADON, ImagIn, ImagOut,  OPT=Opt, rad_header=rad_header
ImagIn = float(ImagIn)
...
filename = 'xx_temp.rad'
p = strpos(filename, '.rad')
if p LT 0 then filename = filename + '.rad'
...
spawn, com
ImagOut = readfits(filename, rad_header)
