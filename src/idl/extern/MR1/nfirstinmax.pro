...
pro nfirstinmax, ima, rec, opt=opt, nfper, nscale=nscale

if not keyword_set(opt) then opt=' -t14 -L '
vs = size(ima)
Nx = vs[1]
Ny = vs[2]
ScaleX  = fix(  alog(float(Nx) / 4. * 3.) / alog(2.))  
ScaleY  = fix(  alog(float(Ny) / 4. * 3.) / alog(2.))  
NbrScale = MIN( [ScaleX, ScaleY])
if keyword_set(nscale) then NbrScale  = nscale  
optn = ' -n ' + STRCOMPRESS(STRING(NbrScale), /REMOVE_ALL)
optWT = opt + Optn
...
mr_transform, ima, wt, opt=optWT, MR_File_Name=MR_File_Name
...
if c gt 0 then wt(index) = 0
writefits, MR_File_Name, wt, header
print, " WT => OPT = ", optWT, "   Total nbr of coeff = ", n
