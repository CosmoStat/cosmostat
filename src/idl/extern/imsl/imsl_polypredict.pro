; $Id: //depot/idl/IDL_70/idl_src/libs/imsl/imsl_6.0/lib/imsl_polypredict.pro#1 $
;
...
;
FUNCTION  imsl_polypredict, info_v, $                     ;INPUT structure
...
@imsl_init.pro
   ...
   size_x = IMSL_SIZE(x)
   IF (size_x(0) GT 1) THEN BEGIN 
      message, "X must be a scalaror a 1-D array."
   END
   ...
   IF (KEYWORD_SET(weights) EQ TRUE) THEN BEGIN
     ...
     IF (size_weights(1) NE n_predict) THEN BEGIN
        message, "The length of the weights array be equal to the number of elements of X"
     END
   END
  ...
  IF (size_info_v(1) LT 64) THEN $
    message, "PREDICT_INFO is not valid, it must be exactly as returned by POLYREGRESS"
  ; 
  ...
  IF (ARG_PRESENT(y) EQ TRUE) THEN BEGIN
  ...
  END
   ...
   ;
   err_status = 0L
   MATHSTAT_173, type, err_status, info_v, IMSL_N_ELEMENTS(info_v), x_cvt,  n_predict, $
                    weights_cvt, $
                    confidence_cvt, $
                    y_cvt, $
                    ci_scheffe_spc, $
