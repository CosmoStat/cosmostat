...
pro mrs_msvsts_multichannel_denoising,donnees,solution,trans,support,image_wt_mc,scaling_wt_mc,cdxztab,dcxztab,ddxztab,NbrScale1=NbrScale1,NbrScale2=NbrScale2,niter=niter,beam=beam
...
sigmacd=[2.61089e-07,   0.00166342,  0.000794248,  0.000437837,  0.000349688,  0.000313635]
 ...
 for j=1,nbrscale1 do begin
  ...
  for k=1,nbrscale2 do begin
   ...
   ;if (j LT nbrscale1) AND (k LT nbrscale2) then 
...
tempsol=fltarr(sizeimage,sizemc)
 ...
 for j=0,SizeMC-1 do begin
  tempdj=tempd[*,j]
  mrs_wttrans,tempdj,tempd_wt,NbrScale=NbrScale1+1
  tempd_wt_coef[*,j,*]=tempd_wt.coef
  solutionj=solution[*,j]
  if keyword_set(beam) then begin
   beamj=beam[*,j]
   mrs_convol,solutionj,beamj,solutionjconv
   solutionj=solutionjconv
  endif
  mrs_wttrans,solutionj,solution_wt,NbrScale=NbrScale1+1
  solution_wt_coef[*,j,*]=solution_wt.coef
 end
 tempd_wt_scaling=tempd_wt_coef
 solution_wt_scaling=solution_wt_coef
 for sx=1,nbrscale1-1 do begin
  tempd_wt_scaling[*,*,sx-1]=total(tempd_wt_coef[*,*,sx-1:Nbrscale1],3)
  solution_wt_scaling[*,*,sx-1]=total(solution_wt_coef[*,*,sx-1:NbrScale1],3)
 end
  ...
  for sx=1,nbrscale1 do begin
   print,'sx=',sx
   chxy[*,*]=tempd_wt_scaling[*,*,sx]
    cgxy[*,*]=tempd_wt_coef[*,*,sx-1]
    chxys[*,*]=solution_wt_scaling[*,*,sx]
    cgxys[*,*,sx-1]=solution_wt_coef[*,*,sx-1]
