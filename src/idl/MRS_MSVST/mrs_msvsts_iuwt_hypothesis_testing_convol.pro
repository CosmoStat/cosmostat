...
pro mrs_msvsts_iuwt_hypothesis_testing_convol,image,image_vst,support,image_rec,NbrScale=NbrScale,coef_seuil=coef_seuil,First_Scale=First_Scale,background=background,detect_coef_pos=detect_coef_pos
...
if coef_seuil LT 5 then begin
 if keyword_set(background) then begin
  ...
  if keyword_set(detect_coef_pos) then ind=where(image_vst.coef[*,0] GT 5*sigma[0],taillesup) else ind=where(abs(image_vst.coef[*,0]) GT 5*sigma[0],taillesup)
  if taillesup GT 0 then Support[ind,0]=1
 endelse
endif else begin if keyword_set(background) then begin
  if keyword_set(detect_coef_pos) then ind=where(image_vst.coef[*,0] - background_vst.coef[*,0] GT coef_seuil*sigma[0],taillesup) else ind=where(abs(image_vst.coef[*,0]) - abs(background_vst.coef[*,0]) GT coef_seuil*sigma[0],taillesup)
  if taillesup GT 0 then Support[ind,0]=1
 endif else begin 
  if keyword_set(detect_coef_pos) then ind=where(image_vst.coef[*,0] GT coef_seuil*sigma[0],taillesup) else ind=where(abs(image_vst.coef[*,0]) GT coef_seuil*sigma[0],taillesup)
  if taillesup GT 0 then Support[ind,0]=1
 endelse
endelse
...
for scale=1,NbrScale-2 do begin
 if keyword_set(background) then begin
  if keyword_set(detect_coef_pos) then ind=where(image_vst.coef[*,scale] - background_vst.coef[*,scale] GT coef_seuil*sigma[scale],taillesup) else ind=where(abs(image_vst.coef[*,scale]) - abs(background_vst.coef[*,scale]) GT coef_seuil*sigma[scale],taillesup)
  if taillesup GT 0 then Support[ind,scale]=1
 endif else begin if keyword_set(detect_coef_pos) then ind=where(image_vst.coef[*,scale] GT coef_seuil*sigma[scale],taillesup) else ind=where(abs(image_vst.coef[*,scale]) GT coef_seuil*sigma[scale],taillesup)
  if taillesup GT 0 then Support[ind,scale]=1
 endelse
endfor
...
save,filename='resultat_test_hypothese.xdr',image,image_vst,support,tjaj,tot,image_rec
