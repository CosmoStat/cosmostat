...
;
;-------------------------------------------------------------------------------
pro whitening_signal, NbSensor, NbSource, ObservSig_in, $
                      FileDescript, WinNumber, SigLength, $
                      WhitenObservSig, WhiteningMat, InvWhiteningMat, $
	              DispDebug=DispDebug, FileDebug=FileDebug, $
		      Verbose=Verbose
   ...
   if (Verbose) then $
	  ...
	  ObservSig = ObservSig_in
   for curSensor=0,NbSensor-1 do $
      ObservSig[curSensor,*] =   ObservSig_in[curSensor,*] $
                               - mean(ObservSig_in[curSensor,*])
   ...
   if (DispDebug) then begin
      window, WinNumber, XSize=500, YSize=500, Title='Centered observed Signal'
