pro flexion_moments,img_in,gam,F,G,w,sig2,xc,yc

;; Following Okura et al. (2006) with corrections

;; This code computes the weighted moments of the input postage stamp
;; (img_in) using the weight function determined by halflight.pro and
;; incorporating corrections arising due to weighting the moments as
;; described in detail in Goldberg & Leonard (2007). The user must
;; specify the input image (img_in), the weight function array (w),
;; the width/variance of the weight function (sig2) only. xc, yc, gam,
;; F and G are all output variables. 
