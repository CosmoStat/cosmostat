...
def info(Data,name=0,mask=None):
    if mask is None:
        ...
        if name:
            ...
            print ("Size = ", Data.shape,  "Type = ", Data.dtype, ", Mean = ", np.mean(Data[ind]), " Sigma = ", np.std(Data[ind]), "Min = ", np.min(Data[ind]), " Max = ", np.max(Data[ind]))
...
def tvilut(d, title=None,xtitle=None,ytitle=None,vmin=None,vmax=None,lut=None,fs=None,filename=None):
    if fs is None:
        FigSize = 10
    else:
        FigSize = fs
...
def tvima2(ima1, ima2, tit1='data1', tit2='data2', vmin=None,vmax=None):
    ...
    cax1 = ax1.imshow(ima1, interpolation='none', cmap='inferno', origin='lower', vmin=vmin, vmax=vmax)
    ax1.set_title(tit1), plt.colorbar(cax1, ax=ax1)
    cax2 = ax2.imshow(ima2, interpolation='none', cmap='inferno', origin='lower', vmin=vmin, vmax=vmax)
...
def tvnima(images, ncol=1, nline=None, title = None, scale = 1,vmin=None,vmax=None):
    ...
    if title is None: title = ['Image (%d)' % i for i in range(1,n_images + 1)]
