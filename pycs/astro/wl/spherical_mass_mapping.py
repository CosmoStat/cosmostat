...
def shear2healpix(g1, g2, ra, dec, nside, Average):
    pixels= hp.ang2pix(nside,theta = 0.5*np.pi - np.deg2rad(dec), phi = np.deg2rad(ra))
    bincount = np.bincount(pixels, minlength = hp.nside2npix(nside))
    bincount_weighted_g1 = np.bincount(pixels, minlength = hp.nside2npix(nside), weights=g1)
    bincount_weighted_g2 = np.bincount(pixels, minlength = hp.nside2npix(nside), weights=g2)
    if Average:
...
def wls_massmapping(mg1, mg2, covmat=None, SigmaGaussianNoise=None, NSigma=None, NScale=None, Niter=None, FirstDetectionScale=None, Signal_PowSpec=None, Mask=None, SigmaArcmin=None,remove_files=True,verbose=True, Prefix="xx_ks", opt=None):
    """
    Parameters
    ----------
    datag1 : TYPE
    ...
    if SigmaArcmin is not None: 
        SigmaRad = SigmaArcmin/(60. * 360.) * (np.pi*2)
...
def wls_kaiser_squires(mg1, mg2, SigmaArcmin=None,remove_files=True,verbose=True, Prefix="xx_ks", opt=None):
    optF=' -m1 '
    if not isinstance(opt, type(None)):
        optF= optF + opt
    result_e, result_b = wls_massmapping(mg1, mg2, SigmaArcmin=SigmaArcmin,remove_files=remove_files,verbose=verbose, Prefix=Prefix, opt=optF)
    return result_e, result_b    

def wls_wiener(mg1, mg2, covmat=None, SigmaGaussianNoise=None, Signal_PowSpec=None, Niter=None, Mask=None, SigmaArcmin=None,remove_files=True,verbose=True, Prefix="xx_wiener", opt=None):
    optF=' -m2 '
    if not isinstance(opt, type(None)):
        optF= optF + opt    
    result_e, result_b = wls_massmapping(mg1, mg2, covmat=covmat, SigmaGaussianNoise=SigmaGaussianNoise, Signal_PowSpec=Signal_PowSpec, Niter=Niter, Mask=Mask, SigmaArcmin=SigmaArcmin,remove_files=remove_files,verbose=verbose, Prefix=Prefix, opt=optF)
    return result_e, result_b

def wls_sparse(mg1, mg2, covmat=None, SigmaGaussianNoise=None, NSigma=3, NScale=None, Niter=None, FirstDetectionScale=None, Mask=None, SigmaArcmin=None,remove_files=True,verbose=True, Prefix="xx_sparse_", opt=None):
    optF=' -m3 '
    if not isinstance(opt, type(None)):
        optF= optF + opt    
    result_e, result_b = wls_massmapping(mg1, mg2, covmat=covmat, SigmaGaussianNoise=SigmaGaussianNoise, NSigma=NSigma, NScale=NScale, Niter=Niter, Mask=Mask, FirstDetectionScale=FirstDetectionScale, SigmaArcmin=SigmaArcmin,remove_files=remove_files,verbose=verbose, Prefix=Prefix, opt=optF)
    return result_e, result_b
...
def wls_mcalens(mg1, mg2, covmat=None, SigmaGaussianNoise=None, Signal_PowSpec=None, NSigma=3, NScale=None, Niter=None, FirstDetectionScale=None, Mask=None, SigmaArcmin=None,remove_files=True,verbose=True, Prefix="xx_mcalens_", opt=None):
    optF=' -m4 '
    if not isinstance(opt, type(None)):
        optF= optF + opt 
    
    result_e, result_b = wls_massmapping(mg1, mg2, covmat=covmat, SigmaGaussianNoise=SigmaGaussianNoise, Signal_PowSpec=Signal_PowSpec, NSigma=NSigma, NScale=NScale, Niter=Niter, Mask=Mask, FirstDetectionScale=FirstDetectionScale, SigmaArcmin=SigmaArcmin,remove_files=remove_files,verbose=verbose, Prefix=Prefix, opt=optF)
    C = MCAlens()
    C.map_e = result_e
    C.map_b = result_b
